...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(getWebhookUrl(), {
      ...
      body: JSON.stringify({ ...req.body, action: 'uploadPhoto' }),
    })
    ...
    if (parsed.ok === false) {
      return res.status(502).json({ error: 'Erreur upload photo', details: parsed.error })
    }

    res.status(200).json({ photo: parsed.photo })
  } catch (error) {
